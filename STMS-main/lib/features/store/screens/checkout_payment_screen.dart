...
class _CheckoutPaymentScreenState
  ...
  Future<void> _pay() async {
    ...
    try {
      ...
      if (method == 'card') {
        // Direct Stripe Payment Flow using sk_test (Bypasses backend Stripe creation)
        final profileResponse = await AuthService.getProfile();
        final profileData = profileResponse?['data'] as Map<String, dynamic>?;
        final email = profileData?['email'] as String? ?? 'customer@autocare.com';
        final firstName = profileData?['firstName'] as String? ?? '';
        final lastName = profileData?['lastName'] as String? ?? '';
        final name = '$firstName $lastName'.trim();

        final paid = await StripeService.processDirectPayment(
          amountInCents: (total * 100).toInt(),
          currency: 'egp',
          customerEmail: email,
        ...
        // Mock success result since we handled payment directly
        checkoutResult = {
          'success': true,
          'orderId': 'ORD-${DateTime.now().millisecondsSinceEpoch}',
        };
      } else {
        // For cash or wallet, rely on the backend normally
        checkoutResult = await PaymentService.syncCartAndCheckout(
          cart,
          method,
        );
      }
