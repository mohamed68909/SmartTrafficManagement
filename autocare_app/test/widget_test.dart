...
void main() {
  ...
  group('CartNotifier', () {
    ...
    test('updates quantity correctly', () {
      final notifier = container.read(cartProvider.notifier);
      notifier.addItem(testProduct);
      notifier.updateQuantity('TEST-001', 5);
    ...
    test('removes item when quantity set to 0', () {
      final notifier = container.read(cartProvider.notifier);
      notifier.addItem(testProduct);
      notifier.updateQuantity('TEST-001', 0);
      expect(container.read(cartProvider), isEmpty);
    });
    ...
    test('calculates subtotal correctly', () {
      final notifier = container.read(cartProvider.notifier);
      notifier.addItem(testProduct);
      notifier.addItem(testProduct); // qty=2, price=500 → total=1000
